...
gen never_coffee = (COFFEENOTP_A == 0)
egen never_coffee_prop = mean(never_coffee), by(AGEP_A)

gen never_friredp = (FRIESTP_A == 0)
egen never_friredp_prop = mean(never_friredp), by(AGEP_A)

gen never_pizza = (PIZZATP_A == 0)
egen never_pizza_prop = mean(never_pizza), by(AGEP_A)

twoway (line never_coffee_prop AGEP_A, lcolor(blue) lwidth(medium)) (line never_friredp_prop AGEP_A, lcolor(red) lwidth(medium)) (line never_pizza_prop AGEP_A, lcolor(green) lwidth(medium)), title("Percentage of different Age Groups") ///
     xtitle("Age") ytitle("Rate (%)") ///
     graphregion(margin(small)) ///
     xlabel(18 20 (5) 85) ///
     ylabel(0 "0%" 0.1 "10%" 0.2 "20%" 0.3 "30%" 0.4 "40%" 0.5 "50%" 0.6 "60%" 0.7 "70%", angle(0)) ///
     legend(label(1 "Never Coffee with Sugar") label(2 "Never Fried Potato") label(3 "Never Pizza"))
