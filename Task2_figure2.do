...
* According to Taks2_figure1.do, we just need to cancel one line and change some variables, we could get another line graph
collect clear
sort AGEP_A

gen never_coffee = (SALADTP_A == 1)
egen never_coffee_prop = mean(never_coffee), by(AGEP_A)

gen never_friredp = (OVEGTP_A == 1)
egen never_friredp_prop = mean(never_friredp), by(AGEP_A)



twoway (line never_coffee_prop AGEP_A, lcolor(blue) lwidth(medium)) (line never_friredp_prop AGEP_A, lcolor(red) lwidth(medium)) , title("Percentage of different Age Groups") ///
     xtitle("Age") ytitle("Rate (%)") ///
     graphregion(margin(small)) ///
     xlabel(18 20 (5) 85) ///
     ylabel(0.1 "10%" 0.2 "20%" 0.3 "30%" 0.4 "40%" 0.5 "50%") ///
     legend(label(1 "Eat Salad Daily") label(2 "Eat Vegetables Daily"))
